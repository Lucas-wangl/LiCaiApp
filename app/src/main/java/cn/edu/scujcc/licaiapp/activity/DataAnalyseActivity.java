...
//定义对象
    ...
    private void inComeData() {
       //第一部分：获取数据
        ...
        while(cursor.moveToNext()){
            ...
            if(mytype.equals("学习-奖金")){//根据不同类型进行判断，把获取的金额放在对应类型金额变量中。
                xxjjmoney+=mymoney;
       ...
       //第二部分：LineChart图表初始化设置X、Y轴
        ...
        List<Entry>   inentries=new ArrayList<>();//Y轴的数据。定义数组，将八个变量都放入数组中。
        ...
        lineDataSet.setDrawFilled(true);//设置折线图内部填充
    ...
    private void outComeData() {
        ...
        //第四部分：曲线展示
        LineData data=new LineData(lineDataSet);//创建LineData对象 属于LineChart折线图的数据集合
        outpay_chart.setData(data);// 将数据集添加到图表中
    }
